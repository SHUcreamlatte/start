	...
	try {
		...
		if (conn != null) {
			...
			if(rs.next()){
				...
				hobby = rs.getString("mem_hobby").split(" ");
...
<body>
    ...
    <form action="info_ok.jsp" name="regform" id="regform" method="post" onsubmit="return sendit()">
        ...
        <p>주민등록번호 : <input type="text" name="ssn1" id="ssn1" maxlength="6" value=<%=ssn1%>> - <input type="password" name="ssn2" id="ssn2" value=<%=ssn2%>> <input type="button" value="주민등록번호 검증" id="ssnBtn"></p>
        <p><label>우편번호 : <input type="text" name="zipcode" id="sample6_postcode" readonly value=<%=zipcode%>></label> <input type="button" value="우편번호 검색" onclick="sample6_execDaumPostcode()"></p>
